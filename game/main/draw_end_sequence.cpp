...
static constexpr char text1[] = "Campaign Complete!";
static constexpr char text2[] = "Thanks for playing!";
...
void drawEndSequence(Background &background, FrameFast &frameFast,
  ...
  if (frameFast.state == FrameState::drawnEndSequence) {
    return;
  }
  ...
  frameFast.state = FrameState::drawnEndSequence;
}
