...
This is based on the 6502 Doynamite format, except with the encoding rearranged
to take somewhat better advantage of the 68k architecture.
...
static void output_bits_at(lz_context *ctx, unsigned data, unsigned len,
  ...
  lz_bittag_t *bit;
  offset = 32 - offset;
  for (bit = &ctx->dst_bitbuf[offset - len]; bit < &ctx->dst_bitbuf[offset];
       ++bit) {
    if (data & 1)
      *bit->ptr |= bit->mask;
    data >>= 1;
...
static void read_input(lz_context *ctx, const char *name) {
...

  {
    ...
    ctx->info = malloc(count * (sizeof *ctx->info + sizeof *ctx->src_data));
 ...
 ******************************************************************************/
static inline unsigned costof_run(unsigned run) { return _log2(run) * 2 + 1; }
...
static inline lz_info optimal_parsing_literal(const lz_info *info,
                                              unsigned cursor) {
  unsigned cost;
  lz_info result;
...
static inline lz_info optimal_parsing(const lz_info *info, unsigned cursor,
                                      signed match_offset,
...
static inline signed *hashof(lz_context *ctx, unsigned a, unsigned b) {
  static const unsigned char random[] = {
      };
...
static inline void generate_hash_table(lz_context *ctx) {
  ...
  for (cursor = ctx->src_begin; cursor != src_end; ++cursor) {
    signed *hash_bucket =
        hashof(ctx, src_data[cursor + 0], src_data[cursor + 1]);
...
static inline void find_matches(lz_context *ctx) {
  ...
  while (cursor != src_begin) {
    ...
    hash_bucket = hashof(ctx, src_data[cursor + 0], src_data[cursor + 1]);
    ...
    while (hash_link >= cursor_limit) {
      ...
      if (match_length < match_limit) {
        ...
        if (!memcmp(&src_data[cursor], &src_data[hash_link], i)) {
          ...
          best_match = optimal_parsing(info, cursor, cursor - hash_link,
                                       match_length + 1, i, best_match);
 ...
 ******************************************************************************/
static inline void encode_literals(lz_context *ctx, unsigned cursor,
                                   unsigned length) {
  unsigned bit;
...
static inline void encode_match(lz_context *ctx, signed offset,
                                unsigned length) {
...
static inline void write_output(lz_context *ctx, bool show_trace) {
  ...
  for (cursor = ctx->src_begin; cursor < src_end; cursor += length) {
    ...
    if (length > 0) {
      ...
      if (show_trace) {
        printf("$%04x %smatch(-%u/$%04x, %u bytes)\n", cursor,
               implicit ? "" : "explicit-", offset, cursor - offset, length);
      }
...
static void prepare_offset_lengths(offset_length_t *table, size_t count) {
  ...
  do {
    ...
    if (bits < OFFSET_LENGTH_MIN)
      fatal("offset lengths cannot be narrower than %u bits",
            OFFSET_LENGTH_MIN);
...
static inline bool parse_offset_lengths(const char *text) {
  if (sscanf(text, "%u/%u/%u/%u:%u/%u/%u/%u", &cfg_short_offset[0].bits,
             &cfg_short_offset[1].bits, &cfg_short_offset[2].bits,
             &cfg_short_offset[3].bits, &cfg_long_offset[0].bits,
             &cfg_long_offset[1].bits, &cfg_long_offset[2].bits,
             &cfg_long_offset[3].bits) != 8) {
    return false;
  }
...
static inline void print_statistics(const lz_context *ctx, FILE *file) {
  ...
  fprintf(file,
          ...
          input_size, ctx->stats.output_size, ctx->stats.output_bits,
          100.0 * ctx->stats.output_size / input_size,

          cfg_short_offset[0].base, cfg_short_offset[0].limit,
          ctx->stats.short_freq[0], cfg_short_offset[1].base,
          cfg_short_offset[1].limit, ctx->stats.short_freq[1],
          cfg_short_offset[2].base, cfg_short_offset[2].limit,
          ctx->stats.short_freq[2], cfg_short_offset[3].base,
          cfg_short_offset[3].limit, ctx->stats.short_freq[3],
          cfg_long_offset[0].base, cfg_long_offset[0].limit,
          ctx->stats.long_freq[0], cfg_long_offset[1].base,
          cfg_long_offset[1].limit, ctx->stats.long_freq[1],
          cfg_long_offset[2].base, cfg_long_offset[2].limit,
          ctx->stats.long_freq[2], cfg_long_offset[3].base,
          cfg_long_offset[3].limit, ctx->stats.long_freq[3],

          ctx->stats.match_count, ctx->stats.match_bytes,
          (double)ctx->stats.match_bytes / ctx->stats.match_count,

          ctx->stats.literal_runs, ctx->stats.literal_bytes,
